...
final class BuyView: UIView {
    ...
    private let increamentButton: UIButton = {
        let button = UIButton()
        button.setImage(CapstoneResourcesAsset.arrowUpwardAltArrowUpwardAltSymbol.image.applyingSymbolConfiguration(.init(font: .preferredFont(forTextStyle: .title1))), for: .normal)
    ...
    @objc private func buttonPressed(_ sender: UIButton) {
        UIView.transition(with: sender, duration: 0.3, options: .transitionCrossDissolve, animations: {
            sender.setImage(CapstoneResourcesAsset.arrowUploadProgressArrowUploadProgressSymbol.image.applyingSymbolConfiguration(.init(font: .preferredFont(forTextStyle: .title1))), for: .normal)
        }, completion: nil)
    }
    ...
    @objc private func buttonReleased(_ sender: UIButton) {
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
            UIView.transition(with: sender, duration: 0.3, options: .transitionCrossDissolve, animations: {
                sender.setImage(CapstoneResourcesAsset.arrowUploadReadyArrowUploadReadySymbol.image.applyingSymbolConfiguration(.init(font: .preferredFont(forTextStyle: .title1))), for: .normal)
            }, completion: { _ in
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                    UIView.transition(with: sender, duration: 0.3, options: .transitionCrossDissolve, animations: {
                        sender.setImage(CapstoneResourcesAsset.arrowUpwardAltArrowUpwardAltSymbol.image.applyingSymbolConfiguration(.init(font: .preferredFont(forTextStyle: .title1))), for: .normal)
                    }, completion: nil)
                }
            })
        }
