...
final public class FirebaseManager {
    ...
    public static func fetchCurrentUserData(completion: @escaping (UserModel?, Error?) -> Void) {
    ...
    public static func fetchBuyHistories(completion: @escaping (BuyHistory?, Error?) -> Void) {
        guard let user = Auth.auth().currentUser else {
            completion(nil, NSError(domain: "Firestore", code: -1, userInfo: [NSLocalizedDescriptionKey: "No user logged in"]))
            return
        }
    ...
    public static func saveBuyHistory(_ buyHistory: BuyHistory, completion: @escaping (Error?) -> Void) {
        guard let user = Auth.auth().currentUser else {
